...
- **Blog**: DB-driven (`BlogPost`). `lib/blog.ts` holds category-hub metadata (db name ↔ slug ↔ label) and derives the article TOC and "Key takeaways" from the body — takeaways are the first sentence under each `##` heading, which works because post bodies are written answer-first. Body format is blank-line-separated blocks; the shared `components/BlogBody.tsx` renderer (used by the public article AND the editor's live preview) supports `## `/`### ` headings, `**bold**`, `*italic*`, `[links]()`, `- ` bullet lists and `![alt](url)` images. The admin `BlogEditor` has a toolbar + Preview for these. TOC/takeaways still key off `## ` headings only.
