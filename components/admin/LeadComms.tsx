...
import { leadVars, renderTemplate, waLink, type CommChannel } from "@/lib/comms";

type LeadLite = {
  name: string; company: string | null; email: string | null; whatsapp: string;
...
export default function LeadComms({ leadId, lead, senderName }: { leadId: string; lead: LeadLite; senderName: string }) {
  ...
  const loadComms = useCallback(async () => {
    ...
    if (json.ok) setComms(json.comms);
  }, [leadId]);

  useEffect(() => {
    // Deferred past commit (repo convention) so no setState fires synchronously.
    ...
    <div className="rounded-2xl border border-stone-200 bg-white p-5 dark:border-stone-800 dark:bg-stone-900">
      <div className="flex items-center justify-between">
        ...
        <div className="inline-flex rounded-full border border-stone-200 p-0.5 dark:border-stone-800">
          <button onClick={() => { setChannel("EMAIL"); setTemplateId(""); }} className={tab("EMAIL", channel === "EMAIL")}><Mail size={12} /> Email</button>
          <button onClick={() => { setChannel("WHATSAPP"); setTemplateId(""); }} className={tab("WHATSAPP", channel === "WHATSAPP")}><MessageCircle size={12} /> WhatsApp</button>
        </div>
      </div>

      {channel === "EMAIL" && !lead.email && (
        <p className="mt-3 text-xs text-amber-600">This lead has no email address.</p>
      )}
      ...
      <div className="mt-3 space-y-2">
        ...
        <textarea value={body} onChange={(e) => setBody(e.target.value)} rows={5} placeholder={`Write to ${lead.name}…`} className={inputCls} disabled={channel === "EMAIL" && !lead.email} />
          ...
          <ul className="space-y-2">
            {comms.map((c) => (
              <li key={c.id} className="flex items-start gap-2 text-xs">
                <span className="mt-0.5 text-stone-400">{c.channel === "EMAIL" ? <Mail size={13} /> : <MessageCircle size={13} />}</span>
